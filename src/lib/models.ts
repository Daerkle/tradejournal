...
export interface DashboardStats {
  ...
  bestTrade: number;
  worstTrade: number;
  // Extended Tradezella-style stats
  zellaScore: number;
  consistency: number;
  riskReward: number;
  avgRMultiple: number;
...
export const TradeService = {
  ...
  async getStats(): Promise<DashboardStats> {
    ...
    // Zella Score (composite score 0-100)
    ...
    const zellaScore = Math.round(normalizedWinRate + normalizedProfitFactor + normalizedConsistency + normalizedRiskReward + normalizedDrawdown);
